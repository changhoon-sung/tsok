...
Release archive names include their operating system and architecture. Build
both supported Linux archives, both macOS archives, or the complete release
matrix with:
...
The release matrix contains these archives:
...
from the latest GitHub release. If `$HOME/.local/bin` is not already on your
`PATH`, add this to your shell profile:
...
keys generated by `tsok serve` are base58-encoded structures:

| Field | Size | v2 value |
| --- | ---: | --- |
| Version | 1 byte | `2` |
| Peer type | 1 byte | `0` (CLI) |
| Legacy STUN address length | 1 byte | `0` |
| DERP region | 2 bytes | Selected region ID |
| Receiver public key | 32 bytes | X25519 public key |
| Overlay private key | 32 bytes | X25519 private key |
...
The upstream source from [`coder/wush`](https://github.com/coder/wush) is
dedicated to the public domain under [CC0 1.0](LICENSE). Third-party components
retain their respective licenses. Run `tsok licenses` to show the dependency
license report URL for the exact commit used to build the CLI.
